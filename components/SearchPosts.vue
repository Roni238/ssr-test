<template>
  <div class="search">
    <input v-model="searchParams" @input="updateSearchParams" type="search"/>
...
export default {
  ...
  methods: {
    updateSearchParams() {
      this.$store.commit('postsModule/setSearchParams', this.searchParams)
      this.$router.push(
        { query: { searchParams: this.searchParams, sourceFilters: this.$route.query.sourceFilters },
          params:{page:'1'}
        });
    },
  },
  computed: {
    searchParams: {
      get() {
        return this.$store.getters['postsModule/getSearchParams'];
      },
      set(value) {
        this.$store.commit('postsModule/setSearchParams', value);
      },
    },
  },
};
